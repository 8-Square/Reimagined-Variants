...
public class PaleOakBuffs {

    private PaleOakBuffs() {}
    ...
    private static void ApplyBuff(ServerPlayerEntity player) {
        ...
        boolean InsideDarkOak = biome.isIn(ModBiomeTagProvider.PALE_OAK_BIOMES);
        ...
        if (InsideDarkOak && HasTagItem) {
            ...
            player.addStatusEffect(new StatusEffectInstance(StatusEffects.GLOWING, 219, 0));
        }
        }
    ...
    public static void initialize() {
        ServerTickEvents.END_SERVER_TICK.register(server  ->
                server.getPlayerManager().getPlayerList()
                        .forEach(PaleOakBuffs::ApplyBuff));
    }
}
